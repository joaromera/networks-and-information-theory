...
# Elegidas por intuición. Se puede probar con timeouts mayores, pero
# tardaríamos mas en obtener respuestas.
TIMEOUTS = [0.5, 1.0, 2.5, 5.0]
...
def tcp_scan(ip, dst_port, dst_timeout):
    ...
    if resp is None:
        ...
        if tcp_layer.flags == 0x12:
            ...
            sr1(
                IP(dst=ip) / TCP(dport=ports, flags="AR"),
                verbose=False,
                timeout=dst_timeout * 5,
            )  # fixme: dport. según la cátedra, debería tomar un arreglo de todos los puertos (habría que crear el arreglo), según Garuflax, hay que mandárselo sólo al puerto que está abierto.
        elif tcp_layer.flags == 0x14:
            return ("cerrado", tcp_layer.flags)  # fixme, ver que se devuelvan las flags


def scan(ports, dst_timeout, dict_writer):
    for port in ports:
        udp = str(udp_scan(ip, port, dst_timeout))
        tcp = str(tcp_scan(ip, port, dst_timeout))
        dict_writer.writerow(
            {"port": port, "udp": udp, "tcp": tcp, "timeout": dst_timeout}
        )
        # Re sucio esto porque la funcion no debería saber que esta impri


if __name__ == "__main__":

    writer = csv.DictWriter(sys.stdout, fieldnames=["port", "udp", "tcp", "timeout"])
    writer.writeheader()

    for dst_timeout in TIMEOUTS:
        scan(ports, dst_timeout, writer)
