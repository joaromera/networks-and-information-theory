...
Los primeros dos variaron en las distintas repeticiones del experimento pero el final fue siempre el mismo (ns1.uba.ar-157.92.1.1).
...
% [1]
En un caso se recorrieron tres niveles DNS hasta obtener la respuesta (similar a \ref{fig:dcubaar_a_tree}). En los otros dos se llegó a recorrer cuatro. En \ref{fig:unajedu_c_tree} se muestra el árbol producido por comenzar con el root server c.
...
% [1]
En dos casos cuatro niveles de DNS fue necesario recorrer para obtener la respuesta, en otro caso fueron necesarios solo tres. En \ref{fig:unfedu_b_tree} se muestra el árbol producido por comenzar con el root server b.
...
% [1]
En todas las iteraciones del experimento fueron necesarios 3 niveles de DNS para alcanzar la respuesta. El árbol de consultas generado comenzando desde el root server a se puede ver en \ref{fig:tubingen_a_tree}.
...
% [2]
Uno de los cuatro servidores consultados nunca respondió. Este es deneb.dfn.de (192.76.176.9). Se obtuvo respuesta al hacer ping con este servidor por lo que no está apagado, así que se debe a otro motivo que no responda las consultas DNS que se le hace.
...
\begin{figure}[h]
  ...
  \caption{Distancia geográfica entre servers de correo y web}
  \label{fig:oman}
\end{figure}